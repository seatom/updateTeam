    ...
    return new Promise((resolve) => {
        ...
        $.get(nm, async (error, response, data) => {
            ...
            try {
                ...
                giftMessage += `\n${result.gifts.giftName} 需要积分：${result.gifts.price} 库存：${result.gifts.stockAmount}`;
            } catch (e) {
                $.logErr(e, response);
            } finally {
                resolve();
...
async function login() {
    let config = {
        url: `${host}login/restaurant`,
        body: `{"mthd": "login", "platform": "wx_mini", "userName": "${user}", "password": "${pwd}"}`,
        headers: {
            'Host': 'www.ttljf.com',
        ...
        $.get(ttlget('user/api/my'), async (err, resp, data) => {
            try {
                ...
                if (safeGet(data)) {
                    ...
                    if (data.code === 0) {
                        userName = `${data.data.userName}(${data.data.mobile})`;
                        console.log('\n积分：' + data.data.integral)
                    }
                    message += `\n【我的积分】 ${data.data.integral}`
                } else {

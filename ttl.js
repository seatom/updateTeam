...
let ttlhddArr = [],ttlcount = ''
let ttlhdd= $.isNode() ? (process.env.ttlhdd ? process.env.ttlhdd : "") : ($.getdata('ttlhdd') ? $.getdata('ttlhdd') : "")
...
!(async () => {
  if (typeof $request !== "undefined") {
      ...
      if(!$.isNode()){
          ttlhddArr.push($.getdata('ttlhdd'))
          let ttlcount = ($.getval('ttlcount') || '1');
          for (let i = 2; i <= ttlcount; i++) {
            ttlhddArr.push($.getdata(`ttlhdd${i}`))
            }
    console.log(`------------- 共${ttlhddArr.length}个账号-------------\n`)
      for (let i = 0; i < ttlhddArr.length; i++) {
        if (ttlhddArr[i]) {
          ttlhdd = ttlhddArr[i];
          $.index = i + 1;
...
await my() 
          ...
          if (process.env.ttlhdd && process.env.ttlhdd.indexOf('@') > -1) {
            ttlhddArr = process.env.ttlhdd.split('@');
            console.log(`您选择的是用"@"隔开\n`)
        } else {
            ttlhdds = [process.env.ttlhdd]
        };
        Object.keys(ttlhdds).forEach((item) => {
        if (ttlhdds[item]) {
            ttlhddArr.push(ttlhdds[item])
        }
    })
          console.log(`共${ttlhddArr.length}个cookie`)
	        for (let k = 0; k < ttlhddArr.length; k++) {
                $.message = ""
                ttlhdd = ttlhddArr[k]
                $.index = k + 1;
...
function ttlck() {
  ...
  const ttlhdd = $request.url

if(ttlhdd)    $.setdata(ttlhdd,`ttlhdd${status}`)


$.log(ttlhdd)
ttlhdd = ttlhdd.match(/loginToken=(.*?)&/)[1] 
$.log(ttlhdd)
